...
async function openWordPressSite() {
    const config = vscode.workspace.getConfiguration('cursorLocalhostViewer');
    const wordPressPort = config.get('wordPressPort', 10018);
    ...
    if (!isWordPressRunning) {
        const result = await vscode.window.showWarningMessage(`WordPress na porcie ${wordPressPort} nie odpowiada. Czy chcesz spróbować inny port?`, 'Tak', 'Nie');
        if (result === 'Tak') {
            await openLocalhostUrl();
            return;
        }
...
async function showCommonPortsMenu() {
    const config = vscode.workspace.getConfiguration('cursorLocalhostViewer');
    const commonPorts = config.get('commonPorts', []);
...
async function openInFullBrowser(url) {
    try {
        ...
        const panel = vscode.window.createWebviewPanel('localhostViewer', `🌐 ${url}`, vscode.ViewColumn.One, {
            enableScripts: true,
...
function getFullBrowserContent(url) {
...
async function checkIfServiceRunning(port) {
